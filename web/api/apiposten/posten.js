...
export default async function handler(req, res) {
  ...
  try {
    if (!postId) {
      // Routes zonder postId: werken op alle posten
      if (req.method === 'GET') {
      ...
      // Routes met postId: werken op één post of op statusen van die post
      ...
      if (req.method === 'PUT') {
        // Update statusen van een post
        // Controleer of url /statusen is meegegeven als query-parameter (of in URL path als je routing dat ondersteunt)
        // Omdat je één handler hebt, kun je status update alleen toestaan als er ook iets als "update statusen" gevraagd wordt

        // Check of de URL iets heeft als ?update=statusen of ?action=statusen
        // Of werk met req.url of headers om dat te checken, maar dat wordt lastiger in 1 handler

        // Simpelste is: verwacht dat body de statusen bevat en update die
